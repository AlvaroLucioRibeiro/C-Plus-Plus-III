...
int main()
{
	...
	int i; // Contador
	int aux = 1; // Var. aux para validação
	float area = 0; // Var. aux para soma
	ponto v[10]; // Struct 
	...
	// Desenvolvimento
	ponto x;
	vetor ponto[N]; // Struct 
	while(!fecho_convexo.empty())
	{
		x = *fecho_convexo.begin();
		ponto[aux].x = x.x;
		ponto[aux].y = x.y;
		fecho_convexo.pop_front();
		aux++;
	}
	...
	{
		if(i == aux - 1)
			area = area + sqrt(pow(ponto[i].x - ponto[1].x, 2) + pow(ponto[i].y - ponto[1].y, 2));
		else if(i < aux - 1)
			area = area + sqrt(pow(ponto[i].x - ponto[i + 1].x, 2) + pow(ponto[i].y - ponto[i + 1].y, 2));
	}

	// Saída de dados
	cout << fixed << setprecision(3) << endl;
	cout << "Comprimento total = " << area << " m" << endl;
